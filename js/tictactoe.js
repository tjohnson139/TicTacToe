...
//this functino will roll for random number twice, one for
...
//this functino will conceal the message console from view
...
function winner(winDetected,winCon) {
    if (winDetected == "win") {
        ...
        glowBoard(winCon); // call functino to make the gameboard pulse with colors
    }
}
...
function checkWinCon2(info,squareArray) {
    ...
    if (match4Avatar != undefined && match3Avatar != undefined && match5Avatar != undefined) {
        if (match4Avatar == match3Avatar && match3Avatar == match5Avatar) {
            winDetected = "win";
        }
    }
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") {//if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) {//if verdict is empty then the square is unoccupied
            ...
            if (paintAvatar == "O") {//change these all to statements instead
